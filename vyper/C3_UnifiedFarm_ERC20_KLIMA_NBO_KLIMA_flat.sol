...
// File: contracts/Staking/C3/C3UnifiedFarmTemplate.sol
...
// |                                                                  |
// ====================================================================
// ====================== C3UnifiedFarmTemplate =====================
// ====================================================================
// Migratable Farming contract that accounts for veC3
// Overrideable for UniV3, ERC20s, etc
// New for V2
//      - Multiple reward tokens possible
//      - Can add to existing locked stakes
//      - Contract is aware of proxied veC3
//      - veC3 multiplier formula changed
...
// (Locked, veC3, and UniV3 portions are new)
...
contract C3UnifiedFarmTemplate is Owned, ReentrancyGuard {
    using SafeERC20 for ERC20;

    /* ========== STATE VARIABLES ========== */

    // Instances
    IveC3 private veC3 = IveC3(0x35681D939867bC02973cbdE7eA8FDF8A169A63F1);
    
    // C3 related
    address internal constant c3_address = 0xed4933b6E94FddE8dF3Be3258307dFAC0e8c9A4c;
    bool internal c3_is_token0;
    uint256 private c3PerLPStored;
    ...
    // veC3 related
    uint256 public vec3_boost_scale_factor = uint256(4e18); // E18. 4x = 4e18; 100 / scale_factor = % vec3 supply needed for max boost
    uint256 public vec3_max_multiplier = uint256(2e18); // E18. 1x = 1e18
    uint256 public vec3_per_c3_for_max_boost = uint256(2e18); // E18. 2e18 means 2 veC3 must be held by the staker per 1 FRAX
    mapping(address => uint256) internal _vec3MultiplierStored;
    mapping(address => bool) internal valid_vec3_proxies;
    ...
    constructor (
        ...
        // Update the c3PerLPStored
        c3PerLPStored = c3PerLPToken();
    }
    ...
    function rewardRates(uint256 token_idx) public view returns (uint256 rwd_rate) {
        ...
        if (gauge_controller_address != address(0)) {
            rwd_rate = (IC3GaugeController(gauge_controller_address).global_emission_rate() * last_gauge_relative_weights[token_idx]) / 1e18;
        }
        else {
    ...
    function userStakedC3(address account) public view returns (uint256) {
        return (c3PerLPStored * _locked_liquidity[account]) / MULTIPLIER_PRECISION;
    }

    function proxyStakedC3(address proxy_address) public view returns (uint256) {
        return (c3PerLPStored * proxy_lp_balances[proxy_address]) / MULTIPLIER_PRECISION;
    }

    // Max LP that can get max veC3 boosted for a given address at its current veC3 balance
    function maxLPForMaxBoost(address account) external view returns (uint256) {
        return (veC3.balanceOf(account) * MULTIPLIER_PRECISION * MULTIPLIER_PRECISION) / (vec3_per_c3_for_max_boost * c3PerLPStored);
    }

    // Meant to be overridden
    function c3PerLPToken() public virtual view returns (uint256) {
    ...
    function minVeC3ForMaxBoost(address account) public view returns (uint256) {
        return (userStakedC3(account) * vec3_per_c3_for_max_boost) / MULTIPLIER_PRECISION;
    }

    function minVeC3ForMaxBoostProxy(address proxy_address) public view returns (uint256) {
        return (proxyStakedC3(proxy_address) * vec3_per_c3_for_max_boost) / MULTIPLIER_PRECISION;
    }

    function veC3Multiplier(address account) public view returns (uint256 vec3_multiplier) {
        // Use either the user's or their proxy's veC3 balance
        uint256 vec3_bal_to_use = 0;
        address the_proxy = staker_designated_proxies[account];
        vec3_bal_to_use = (the_proxy == address(0)) ? veC3.balanceOf(account) : veC3.balanceOf(the_proxy);

        // First option based on fraction of total veC3 supply, with an added scale factor
        uint256 mult_optn_1 = (vec3_bal_to_use * vec3_max_multiplier * vec3_boost_scale_factor)
                            / (veC3.totalSupply() * MULTIPLIER_PRECISION);
        
        // Second based on old method, where the amount of FRAX staked comes into play
        uint256 mult_optn_2;
        {
            uint256 veC3_needed_for_max_boost;

            // Need to use proxy-wide FRAX balance if applicable, to prevent exploiting
            veC3_needed_for_max_boost = (the_proxy == address(0)) ? minVeC3ForMaxBoost(account) : minVeC3ForMaxBoostProxy(the_proxy);

            if (veC3_needed_for_max_boost > 0){
                uint256 user_vec3_fraction = (vec3_bal_to_use * MULTIPLIER_PRECISION) / veC3_needed_for_max_boost;
                
                mult_optn_2 = (user_vec3_fraction * vec3_max_multiplier) / MULTIPLIER_PRECISION;
            }
            else mult_optn_2 = 0; // This will happen with the first stake, when user_staked_c3 is 0
        }

        // Select the higher of the two
        vec3_multiplier = (mult_optn_1 > mult_optn_2 ? mult_optn_1 : mult_optn_2);
    ...
    // Proxy can allow a staker to use their veC3 balance (the staker will have to reciprocally toggle them too)
    // Must come before stakerSetVeC3Proxy
    function proxyToggleStaker(address staker_address) external {
        require(valid_vec3_proxies[msg.sender], "Invalid proxy");
    ...
    // Staker can allow a veC3 proxy (the proxy will have to toggle them first)
    function stakerSetVeC3Proxy(address proxy_address) external {
        require(valid_vec3_proxies[proxy_address], "Invalid proxy");
    ...
    function retroCatchUp() internal {
        ...
        for (uint256 i = 0; i < rewardDistributors.length; i++){ 
            ...
            if (reward_distributor_address != address(0)) {
                IC3GaugeC3RewardsDistributor(reward_distributor_address).distributeReward(address(this));
            }
        }
        ...
        for (uint256 i = 0; i < gaugeControllers.length; i++){ 
            ...
            if (gauge_controller_address != address(0)) {
                if (force_update || (block.timestamp > last_gauge_time_totals[i])){
                    ...
                    last_gauge_relative_weights[i] = IC3GaugeController(gauge_controller_address).gauge_relative_weight_write(address(this), block.timestamp);
                    last_gauge_time_totals[i] = IC3GaugeController(gauge_controller_address).time_total();
                }
            }
        }
    }
    ...
    // Adds a valid veC3 proxy address
    function toggleValidVeC3Proxy(address _proxy_addr) external onlyByOwnGov {
        valid_vec3_proxies[_proxy_addr] = !valid_vec3_proxies[_proxy_addr];
    }
    ...
    function setMiscVariables(
        ...
        // [1] uint256 _vec3_max_multiplier,
        // [2] uint256 _vec3_per_c3_for_max_boost,
        // [3] uint256 _vec3_boost_scale_factor,
        ...
        vec3_max_multiplier = _misc_vars[1];
        vec3_per_c3_for_max_boost = _misc_vars[2];
        vec3_boost_scale_factor = _misc_vars[3];
...
// File: contracts/Staking/C3/C3_UnifiedFarm_ERC20.sol
...
contract C3UnifiedFarm_ERC20 is C3UnifiedFarmTemplate {
    ...
    C3UnifiedFarmTemplate(_owner, _rewardTokens, _rewardManagers, _rewardRatesManual, _gaugeControllers, _rewardDistributors)
    {
        ...
        // c3_is_token0 = token0 == c3_address;
        ...
        if (token0 == c3_address) c3_is_token0 = true;
    ...
    function c3PerLPToken() public view override returns (uint256) {
        ...
        //     uint256 total_c3_reserves = c3_is_token0 ? reserve0 : reserve1;

        //     c3_per_lp_token = (total_c3_reserves * 1e18) / stakingToken.totalSupply();
        // }
        ...
        //     (, IFeederPool.BassetData memory vaultData) = (stakingToken.getBasset(c3_address));
        //     total_c3_reserves = uint256(vaultData.vaultBalance);
        //     c3_per_lp_token = (total_c3_reserves * 1e18) / stakingToken.totalSupply();
        // }
        ...
        // ============================================
        // {
        //    uint256 c33crv_held = stakingToken.totalUnderlyingControlled();
        
        //    // Optimistically assume 50/50 FRAX/3CRV ratio in the metapool to save gas
        //    c3_per_lp_token = ((c33crv_held * 1e18) / stakingToken.totalSupply()) / 2;
        // }

        // StakeDAO Vault
        // ============================================
        // {
        //    uint256 c33crv_held = stakingToken.balance();
        ...
        // Vesper
        // ============================================
        //c3_per_lp_token = stakingToken.pricePerShare();
        ...
        )
    {
        ...
        if (_locked_liquidity[account] == 0 && _combined_weights[account] == 0) {
            ...
            midpoint_vec3_multiplier = (new_vec3_multiplier + _vec3MultiplierStored[account]) / 2;
        }
        ...
        for (uint256 i = 0; i < lockedStakes[account].length; i++) {
            ...
            uint256 combined_boosted_amount = (liquidity * (lock_multiplier + midpoint_vec3_multiplier)) / MULTIPLIER_PRECISION;
...
// File: contracts/Staking/C3_Variants/C3_UnifiedFarm_ERC20_KLIMA_NBO_KLIMA.sol


pragma solidity >=0.8.0;


contract C3UnifiedFarm_ERC20_KLIMA_NBO_KLIMA is C3UnifiedFarm_ERC20 {
    constructor (
        address _owner,
        address[] memory _rewardTokens,
        address[] memory _rewardManagers,
